...
const instructions = [[
  {
    items: [
      ...
      {textItems: [({stimRandomizer}) => (stimRandomizer.name === "blackBoxes" ? "" : `There are three phases in this study; You are participating in the second phase. Recently, during phase one, CU Boulder students came into the lab and we took their photos.`)]},
      {textItems: [({stimRandomizer}) => (stimRandomizer.name === "blurryFaces" ? "You will see their blurred photos during some parts of today’s study." : "")]},
      {textItems: [({stimRandomizer}) => (stimRandomizer.name === "faces" ? "You will see their photos during some parts of today’s study." : "")]},
      {textItems: [({stimRandomizer}) => (stimRandomizer.name === "blackBoxes" ? "" : `After you finish with the second phase of the study today, those CU Boulder students from phase one will return to take part in phase three.`)]},
    ...
    items: [
      ...
      {textItems: [({stimRandomizer}) => (stimRandomizer.name === "blackBoxes" ? `(2) In the second task you will take part in a decision task. You will be presented with two different Bayesian based machine learning algorithms, which we will call “black boxes”. The black boxes represent algorithms based on data from previous students who took part in this decision task. In this task, you will choose between these two black boxes to give hypothetical sums of money to (e.g., $1).` : `(2) In the second task you will take part in a decision task. You will be randomly paired with two CU Boulder students from phase one and then be asked to choose which to give hypothetical sums of money to (e.g., $1).`)]},
    ],
    timeout: 10000,
    prompt: defaultPrompt,
  }, {
    ...
    items: [
      {textItems: [({stimRandomizer}) => (stimRandomizer.name === "faces" ? "While you are doing the search task, two photographs of previous CU Boulder students will also appear. When the letters disappear, the photos will remain on the screen for the decision task." : "")]},
      {textItems: [({stimRandomizer}) => (stimRandomizer.name === "blurryFaces" ? "While you are doing the search task, two photographs of previous CU Boulder students with blurred faces will also appear. When the letters disappear, the photos will remain on the screen for the decision task." : "")]},
      {textItems: [({stimRandomizer}) => (stimRandomizer.name === "blackBoxes" ? "While you are doing the search task, the two black boxes from phase one will also appear. When the letters disappear, the black boxes will remain on the screen for the decision task." : "")]},
      { textItems: [{component: "strong", text: "The basic layout of the screen will look like this:"}] }
    ],
    ...
    items: [
      {textItems: [({stimRandomizer}) => (stimRandomizer.name === "blackBoxes" ? "" : "Everyone in phase two of this study will be Player One.")]},
      {textItems: [({stimRandomizer}) => (stimRandomizer.name === "blackBoxes" ? "To begin, you will be given $1.00. You will then choose which black box you want to send your $1.00 to. Your money will then be multiplied by four and and given to the black box you select." : "To begin, you will be given $1.00. You will then choose who you want to send your $1.00 to. The participant you select will become the Player Two. Your money will then be multiplied by four and placed in the Player Two’s account.")]},
      {textItems: [{component: "strong", text: "The basic layout of the screen will look like this:"}]}
    ],
    ...
    items: [
      ...
      {textItems: [({stimRandomizer}) => (stimRandomizer.name === "blackBoxes" ? "The black box that is selected will perform its mathematical function that converts money inputted into money to be outputted." : "Player Two will return to the lab in the next few weeks to take part in the third phase of the study. They will be able to see everything you saw (including their own photo, the other participant’s photo, and the choice you made).")]},
